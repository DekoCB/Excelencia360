...
use App\Modules\Docentes\Exports\DocentesPlantillaExport;
...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function descargarPlantilla()
    {
        ...
        return Excel::download(new DocentesPlantillaExport, 'plantilla-docentes.xlsx');
    }

    public function procesar(DocenteService $service): void
    {
        Gate::authorize('docentes.gestionar');
...
<div>
    <x-slot name="header">
        <h1 class="font-display text-2xl text-ink">Carga masiva de docentes</h1>
        <p class="mt-1 text-sm text-ink-dim">Registra muchos docentes a la vez subiendo un archivo Excel.</p>
    </x-slot>
        ...
        <div class="rounded-2xl border border-border bg-surface shadow-sm p-6">
            <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p class="mt-1 text-sm text-ink-dim">Un docente por fila. Solo nombres, apellidos y DNI son obligatorios.</p>
                </div>
        ...
        @if ($resultado)
            <div class="rounded-2xl border border-border bg-surface shadow-sm p-6">
                ...
                <div class="mt-3 rounded-md border border-ok/30 bg-ok/10 px-4 py-3 text-sm text-ok">
                    {{ $resultado['exitosos'] }} docente{{ $resultado['exitosos'] === 1 ? '' : 's' }} registrado{{ $resultado['exitosos'] === 1 ? '' : 's' }} correctamente.
                </div>
