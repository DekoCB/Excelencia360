...
/**
 * Caja de búsqueda del topbar, embebida en layout.navigation, anclada a
 * la izquierda junto al logo. No tiene ruta propia -- es un widget que
 * enlaza a las páginas que ya existen en cada módulo (ver
 * BusquedaGlobalService). Colapsada es solo el ícono de la lupa; se
 * expande a un campo de texto al seleccionarla.
 */
...
<div
    class="relative"
    x-data="{ expandido: false, abierto: false }"
    x-on:click.outside="expandido = false; abierto = false; if ($wire.get('termino') !== '') $wire.cerrar()"
    x-on:keydown.escape.window="expandido = false; abierto = false"
>
    <button
        type="button"
        x-show="! expandido"
        x-on:click="expandido = true; $nextTick(() => $refs.campoBusqueda.focus())"
        class="rounded-md p-2 text-ink-dim transition hover:bg-surface-2 hover:text-ink"
        aria-label="Buscar estudiante, docente, personal…"
    >
        <x-heroicon-o-magnifying-glass class="h-5 w-5" />
    </button>

    <div x-show="expandido" x-cloak x-transition.origin.left class="w-64 sm:w-80">
        <div class="relative">
                        ...
                        @foreach ($resultados as $resultado)
                            <li>
                                <a
                                    href="{{ $resultado->url }}"
                                    wire:navigate
                                    x-on:click="abierto = false; expandido = false"
