...
use App\Modules\Personal\Exports\PersonalPlantillaExport;
...
new #[Layout('layouts.app')] class extends Component
{
    ...
    public function descargarPlantilla()
    {
        ...
        return Excel::download(new PersonalPlantillaExport, 'plantilla-personal.xlsx');
    }

    public function procesar(PersonalService $service): void
    {
        Gate::authorize('personal.gestionar');
        ...
        $this->resultado = $service->registrarDesdeFilas($import->filas);
        $this->reset('archivo');
    }
}; ?>
...
<div>
    <x-slot name="header">
        <h1 class="font-display text-2xl text-ink">Carga masiva de personal</h1>
        <p class="mt-1 text-sm text-ink-dim">Registra a mucho personal a la vez subiendo un archivo Excel.</p>
    </x-slot>
        ...
        <div class="rounded-2xl border border-border bg-surface shadow-sm p-6">
            <div class="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                <div>
                    ...
                    <p class="mt-1 text-sm text-ink-dim">Una persona por fila. Nombres, apellidos, DNI y cargo son obligatorios.</p>
                </div>
        ...
        @if ($resultado)
            <div class="rounded-2xl border border-border bg-surface shadow-sm p-6">
                ...
                <div class="mt-3 rounded-md border border-ok/30 bg-ok/10 px-4 py-3 text-sm text-ok">
                    {{ $resultado['exitosos'] }} persona{{ $resultado['exitosos'] === 1 ? '' : 's' }} registrada{{ $resultado['exitosos'] === 1 ? '' : 's' }} correctamente.
                </div>
